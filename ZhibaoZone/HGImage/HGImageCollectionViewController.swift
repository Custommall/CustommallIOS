    ...
    func collectionView(_ collectionView: UICollectionView,
        ...
        self.imageManager.requestImage(for: asset, targetSize: assetGridThumbnailSize,
            ...
            if asset.mediaType == .video{
                let transparentBackground:UIView = UIView.init(frame: CGRect(x: 0, y: cell.imageView.bounds.height - 20, width: cell.imageView.bounds.width, height: 20))
                transparentBackground.backgroundColor = UIColor.clear
                
                let transparentLayer:UIView = UIView.init(frame: CGRect(x: 0, y: 0, width: cell.imageView.bounds.width, height: 20))
                let videoView:UIImageView = UIImageView.init(frame: CGRect(x: 5, y: 0, width: 20, height: 20))
                videoView.image = UIImage(named:"videoicon-white")
                
                transparentLayer.alpha = 0.4
                transparentLayer.backgroundColor = UIColor.white
                
                let videoDurationLabel:UILabel = UILabel.init(frame: CGRect(x: 30, y: 0, width: 60, height: 20))
                ...
                let videoDurationValueInSeconds = Int(asset.duration)
                let mins = Int(videoDurationValueInSeconds/60)
                let seconds = videoDurationValueInSeconds%60
                let formatedString = String(format:"%02d:%02d",mins,seconds+1)
                videoDurationLabel.text = formatedString
                
                transparentBackground.addSubview(transparentLayer)
                transparentBackground.addSubview(videoView)
                transparentBackground.addSubview(videoDurationLabel)
                
                
                
                cell.imageView.addSubview(transparentBackground)

                if cell.imageView.subviews.count == 2{
                    cell.imageView.subviews[0].isHidden = true
                }
            }
        }
